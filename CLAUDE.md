...
│   ├── data/
│   │   ├── graph.json     # 大阪近郊区間 主要路線データ（358駅・371エッジ・18路線ID）
...
大阪環状線(O)・JR京都線＋琵琶湖線(A)・JR神戸線(A)・北陸本線(A)・JR宝塚線(G)・JR東西線(H)・学研都市線(H)・大和路線(Q)・おおさか東線(F)・阪和線(R)・奈良線(D)・湖西線(B)・草津線(C)・関西本線非電化(V)・嵯峨野線(E)・万葉まほろば線(U)・和歌山線(T)・関西空港線(S)・羽衣支線(HA)・加古川線(I)・JRゆめ咲線(P)・山陽本線 姫路〜相生(A)
